...
macro_rules! serde_workaround {
    (
        ...
        const _: () = {
            ...
            #[derive(FromRepr, IntoStaticStr, EnumString, Clone, Copy, Debug)]
            ...
            impl<'de> serde::de::Visitor<'de> for Visitor {
                    ...
                    A: MapAccess<'de>,
                {
                    ...
                    while let Some(key) = map.next_key::<Ident>().or(Ok(Some(Ident::Unknown)))? {
                        match key {
                            $(
...
macro_rules! serde_deserialize_with {
    ($key:ident; $field:ident; $map:ident; $ty:ty; $de_with:path; $name:ident) => {{
        ...
        $crate::serde_workaround::check_is_already_set($key, &$field, &$map)?;
        $field = Some($map.next_value::<__DeserializeWith<'de>>()?.value);
    }};
    ($key:ident; $field:ident; $map:ident) => {
        $crate::serde_workaround::set_if_none($key, &mut $field, &mut $map)?
    };
}
